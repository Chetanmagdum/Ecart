...
def remove_order(request, oid):
    od = Order.objects.filter(id = oid)
    od.delete()
    context ={}
    context['data'] = od
    return redirect("/viewcart")
...
def user_logout(request):
    logout(request)
    return render(request, "user_   login")
...
def search(request):
    ...
    allproduct = pname.union(pdetails, pcat)
    if len(allproduct) == 0:
        context['err msg'] = "Product not found"
        
    context['data'] = allproduct  
    return render(request, "index.html", context)